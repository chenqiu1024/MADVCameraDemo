...
static int mov_read_lutz(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    c->fc->lutz.size = atom.size;
    c->fc->lutz.offset = avio_tell(pb);
    
    av_log(c->fc, AV_LOG_ERROR, "new method: lutz size: sz: %"PRId64" %"PRId64"\n",
           c->fc->lutz.size , c->fc->lutz.offset);
    return 0;
}

static int mov_read_fltr(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    av_log(c->fc, AV_LOG_ERROR, "mov_read_fltr");
    uint8_t fltr[4];
    avio_read(pb, fltr, 4);
    c->fc->gyro_dummy_lines = fltr[0] | fltr[1] << 8 | fltr[2] << 16 | fltr[3] << 24;
    
    av_log(c->fc, AV_LOG_ERROR, "gyro dummy line number = %d", c->fc->gyro_dummy_lines);
    return 0;
}

static int mov_read_cutl(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    av_log(c->fc, AV_LOG_ERROR, "mov_read_cutl");
    c->fc->cut_count = 0;
    c->fc->cut_size_per_point = 4;
    
    int cutCount = 0;
    if (atom.size == ((4+4)*20 + 4)) {
        uint8_t readData[4];
        avio_read(pb, readData, 4);
        cutCount = readData[0] | readData[1] << 8 | readData[2] << 16 | readData[3] << 24;
        if (cutCount > 0){
            c->fc->cut_count = cutCount;
            c->fc->cut_size_per_point = 4;
            uint8_t* cutOrigData = malloc((4+4)*20);
            avio_read(pb, cutOrigData, (4+4)*20);
            
            c->fc->cut_data = malloc(4*cutCount);
            av_log(c->fc, AV_LOG_ERROR, "cutCount = %d", cutCount);
            for (int i = 0; i < cutCount; i++) {
                memcpy(c->fc->cut_data + i * 4, cutOrigData + i * 8, 4);
                av_log(c->fc, AV_LOG_ERROR, "%d : %d", i, (c->fc->cut_data[i*4] | c->fc->cut_data[i*4+1] << 8 | c->fc->cut_data[i*4+2] << 16 | c->fc->cut_data[i*4+3] << 24));
            }
            free(cutOrigData);
        }
    }
...
static int mov_read_tlyd(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    av_log(c->fc, AV_LOG_ERROR, "mov_read_tlyd");
    
    c->fc->gyro_size_per_frame = 36;
    uint8_t* gyroData = malloc(atom.size);
    avio_read(pb, gyroData, atom.size);
    
    int offset = 0;
    for (int i = 0; i < atom.size / c->fc->gyro_size_per_frame; i++) {
        boolean_t validData = FALSE;
        for (int j = 0; j < c->fc->gyro_size_per_frame; j++) {
            if (gyroData[offset + j] != 0) {
                validData = TRUE;
                break;
            }
        }
        if (validData == FALSE) {
            av_log(c->fc, AV_LOG_ERROR, "got original valid gyro data size: %"PRId32" data: %02x %02x %02x %02x ... \n",
                   offset, gyroData[0], gyroData[1], gyroData[2], gyroData[3]);
            break;
        }
        offset += c->fc->gyro_size_per_frame;
    }
    
    c->fc->gyro_size = offset;
    if (c->fc->gyro_size > 0) {
        c->fc->gyro_data = malloc(c->fc->gyro_size + c->fc->gyro_dummy_lines * c->fc->gyro_size_per_frame);
        
        int targetOffset = 0;
        for (int i = 0; i < c->fc->gyro_dummy_lines; i++) {
            
            memcpy(c->fc->gyro_data + targetOffset, gyroData, c->fc->gyro_size_per_frame );
            targetOffset += c->fc->gyro_size_per_frame;
        }
        memcpy(c->fc->gyro_data + targetOffset, gyroData, c->fc->gyro_size);
        c->fc->gyro_size += c->fc->gyro_size_per_frame * c->fc->gyro_dummy_lines;
        
        av_log(c->fc, AV_LOG_ERROR, "gyro size = %"PRId32" gyro data: %02x %02x %02x %02x ... \n",
            c->fc->gyro_size, c->fc->gyro_data[0], c->fc->gyro_data[1], c->fc->gyro_data[2], c->fc->gyro_data[3]);
    }
...
static int mov_read_GYRA(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    av_log(c->fc, AV_LOG_ERROR, "mov_read_GYRA");
    c->fc->gyro_size_per_frame = 36;
    uint8_t* gyroData = malloc(atom.size);
    avio_read(pb, gyroData, atom.size);
    ...
    c->fc->gyro_size = offset;
    if (c->fc->gyro_size > 0) {
        c->fc->gyro_data = malloc(c->fc->gyro_size);
        
        memcpy(c->fc->gyro_data, gyroData, c->fc->gyro_size);
        av_log(c->fc, AV_LOG_ERROR, "gyro size = %"PRId32" gyro data: %02x %02x %02x %02x ... \n",
               c->fc->gyro_size, c->fc->gyro_data[0], c->fc->gyro_data[1], c->fc->gyro_data[2], c->fc->gyro_data[3]);
    }
...
static int mov_read_DISP(MOVContext *c, AVIOContext *pb, MOVAtom atom){
    av_log(c->fc, AV_LOG_ERROR, "mov_read_DISP");
           
    c->fc->disp_mode = 0x03;
    
    uint8_t DISP[4];
    avio_read(pb, DISP, 4);
    c->fc->disp_mode = DISP[0] | DISP[1] << 8 | DISP[2] << 16 | DISP[3] << 24;
    av_log(c->fc, AV_LOG_ERROR, "disp_mode = %"PRId32"\n", c->fc->disp_mode);
    return 0;
}
...
static int mov_read_moov(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->found_moov) {
        ...
        c->fc->moov_box_size_offset = avio_tell(pb) - 8;
        av_log(c->fc, AV_LOG_ERROR, "moov_box_size_offset: %"PRId64"\n", c->fc->moov_box_size_offset);
    }
...
{
    av_log(mov->fc, AV_LOG_ERROR, "mov_build_index\n");
    ...
    if (!(st->codec->codec_type == AVMEDIA_TYPE_AUDIO &&
            ...
            for (j = 0; j < sc->stsc_data[stsc_index].count; j++) {
                ...
                if (sc->pseudo_stream_id == -1 ||
                   sc->stsc_data[stsc_index].id - 1 == sc->pseudo_stream_id) {
                    ...
                    /*
                    av_log(mov->fc, AV_LOG_ERROR, "AVIndex stream %d, sample %d, offset %"PRIx64", dts %"PRId64", ts %"PRId64", "
                            "size %d, distance %d, keyframe %d\n", st->index, current_sample,
                            current_offset, current_dts, e->timestamp, sample_size, distance, keyframe);*/
        ...
        for (i = 0; i < sc->chunk_count; i++) {
            ...
            while (chunk_samples > 0) {
                ...
                /*
                av_log(mov->fc, AV_LOG_ERROR, "AVIndex stream %d, chunk %d, index %d, offset %"PRIx64", dts %"PRId64", ts %"PRId64", "
                       "size %d, duration %d\n", st->index, i, st->nb_index_entries-1, current_offset, current_dts, e->timestamp, 
                       size, samples);*/
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if(!c->fc->bVideoTrakFound) {
        c->fc->bVideoTrakFound = TRUE;
        c->fc->video_trak_box_size_offset = avio_tell(pb) - 8;
        c->fc->video_trak_box_end_offset = avio_tell(pb) + atom.size;
        av_log(c->fc, AV_LOG_ERROR, "video_trak_box_size_offset: %"PRId64" video_trak_box_end_offset: %"PRId64"\n", c->fc->video_trak_box_size_offset, c->fc->video_trak_box_end_offset);
    }
...
{ MKTAG('m','a','d','v'), mov_read_default },
{ MKTAG('l','u','t','z'), mov_read_lutz },
...
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    while (total_size + 8 <= atom.size && !url_feof(pb)) {
        ...
        if (a.type == MKTAG('l','u','t','z')) {
            av_log(c->fc, AV_LOG_ERROR, "old method: lutz size: sz: %"PRId64" %"PRId64"\n",
                   a.size-8, avio_tell(pb));
        }
        
        if (a.type == MKTAG('m','a','d','v')) {
            c->fc->is_madv_content = TRUE;
            av_log(c->fc, AV_LOG_ERROR, "Found MadV box\n");
        }
        
        av_log(c->fc, AV_LOG_ERROR, "type: %08x '%.4s' parent:'%.4s' sz: %"PRId64" %"PRId64" %"PRId64"\n",
...
{
    //av_log(s, AV_LOG_ERROR, "mov_read_header");
...
{
    //av_log(s, AV_LOG_ERROR, "mov_read_packet");
    MOVContext *mov = s->priv_data;
    MOVStreamContext *sc;
    AVIndexEntry *sample;
    AVStream *st = NULL;
...
{
    //av_log(s, AV_LOG_ERROR, "mov_read_seek");
    AVStream *st;
