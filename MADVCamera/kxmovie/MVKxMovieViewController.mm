...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    if(!self.isFinishEncoder)
    {
        ...
        NSLog(@"EAGLContext : MVKxMovieViewController $ viewDidAppear # _glView = %lx, glRenderLoop = %lx", _glView.hash, _glView.glRenderLoop.hash);
...
//    [_activityIndicatorView stopAnimating];
    ...
    _buffered = NO;
    _interrupted = YES;
    NSLog(@"EAGLContext(%@) : MVKxMovieViewController $ viewWillDisappear # Before MVGLView stopRendering @ %lx, glRenderLoop = %lx", [NSThread currentThread],  self.glView.hash, self.glView.glRenderLoop.hash);
...
- (void) setupPresentView
{
    ...
    if (_glView != presentView)
    {
        if (_glView)
        {
            ...
            NSLog(@"EAGLContext : MVKxMovieViewController $ setupPresentView # set _glView = nil (%@), PresentView = %@, glRenderLoop = %lx", _glView, presentView, _glView.glRenderLoop.hash);
            _glView = nil;
        }
        _glView = (MVGLView*) presentView;
        NSLog(@"EAGLContext : MVKxMovieViewController $ setupPresentView # set _glView = PresentView = %@, glRenderLoop = %lx", presentView, _glView.glRenderLoop.hash);
...
- (void) tick
{
    //NSLog(@"MVKxMovieViewController :: tick");
