...
- (FDSPutObjectResult *)putObject:(NSString *)objectName
    intoBucket:(NSString *)bucketName fromInputStream:(NSInputStream *)input
    withObjectMetadata:(FDSObjectMetadata *)metadata andParams:(NSArray *)params
    andProgressListener:(FDSProgressListener *)listener baseurl:(NSString *)baseurl signatures:(NSArray *)signatures completeurl:(NSString *)completeurl {
  [FDSArgs notEmpty:bucketName forName:@"bucket name"];
...
//    uploadId = initMultipartUploadResult.uploadId;
    ...
    FDSUploadThreadObj *threadObj = [[FDSUploadThreadObj alloc]
        ...
        objectLength:contentLength  partSize:_config.uploadPartSize baseurl:baseurl signatures:signatures completeurl:completeurl];
    ...
    @autoreleasepool {
      for (int i = 0; i < _config.uploadThreadNumber - 1 ; i++) {
        NSThread *thread = [[NSThread alloc] initWithTarget:threadObj
          ...
          NSLog(@"%@",[NSString stringWithFormat:@"Thread%d", i]);
    ...
    uploadPartResultList.uploadPartResultList = threadObj.results;
      if (signatures && signatures.count>0) {
          return [self completeMultipartUpload:objectName intoBucket:bucketName
                                     ...
                                     andParams:params completeurl:completeurl];
      }else
      {
          return nil;
      }
...
//        uint8_t buffer[FDSClientConfiguration.DEFAULT_PART_SIZE];
        uint8_t* buffer = (uint8_t*) malloc(FDSClientConfiguration.DEFAULT_PART_SIZE);
...
//                               bucketName, objectName, uploadId, index];
        ...
        {
            uriString=[NSString stringWithFormat:@"%@&Signature=%@&partNumber=%d",baseurl,signatures[index-1],index];
        }
        ...
        while (YES) {
            @try {
                ...
                NSArray * baseArr = [baseurl componentsSeparatedByString:@"?"];
                NSString * type;
                if (baseArr.count >0) {
                    type = baseArr[0];
                    type = [type lastPathComponent];
                    type = [type pathExtension];
                }
                NSLog(@"++++++++type%@",type);
                ...
                if (!(result && result.etag && result.partSize) && !(result && result.accessKeyId && result.bucketName && result.expires && result.objectName && result.signature)) {
...
- (FDSPutObjectResult *)completeMultipartUpload:(NSString *)objectName
    ...
    andResultList:(FDSUploadPartResultList *)resultList
                                      andParams:(NSArray *)params completeurl:(NSString *)completeurl {
  NSString *uriString = completeurl;
//  if (params) {
...
//  }
    ...
    NSArray * baseArr = [completeurl componentsSeparatedByString:@"?"];
    NSString * type;
    if (baseArr.count >0) {
        type = baseArr[0];
        type = [type lastPathComponent];
  ...
  int retriedTimes = 0;
    while (YES) {
        @try {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:UPLOAD_SUCCESS object:objectName];
            return result;
        } @catch (GalaxyFDSClientException *e) {
             if (++retriedTimes >= _config.maxRetryTimes)
             {
