        ...
        @autoreleasepool //write by spy
        {
            ...
            {//NSLog(@"VideoLeak : new renderSource coming : %@", renderSource);
                ...
                else if ([renderSource isKindOfClass:NSClassFromString(@"KxVideoFrame")])
                {
                    ...
                    IMP widthImp = [renderSource methodForSelector:widthSelector];
                    NSUInteger (*widthFunc)(id, SEL) = (NSUInteger(*)(id,SEL)) widthImp;
                    NSUInteger frameWidth = widthFunc(renderSource, widthSelector);
                    
                    SEL heightSelector = NSSelectorFromString(@"height");
                    IMP heightImp = [renderSource methodForSelector:heightSelector];
                    NSUInteger (*heightFunc)(id, SEL) = (NSUInteger(*)(id,SEL)) heightImp;
...
- (void) dealloc {
    NSLog(@"EAGLContext : MVGLView dealloc %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
}
...
- (void) willDisappear {
    NSLog(@"EAGLContext : MVGLView willDisappear @ %lx, glRenderLoop = %lx", self.hash, self.glRenderLoop.hash);
    ///!!![self stopMotionManager];
...
- (void) willAppear {
    NSLog(@"EAGLContext : MVGLView willAppear @ %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
...
- (void) willMoveToSuperview:(UIView *)newSuperview {
    if (newSuperview == nil)
    {
        NSLog(@"EAGLContext : MVGLView willMoveToSuperview (nil) @ %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
        [self willDisappear];
    }
    else
    {
        NSLog(@"EAGLContext : MVGLView willMoveToSuperview @ %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
    }
}
...
- (void) didMoveToSuperview {
    if (self.superview)
    {
        NSLog(@"EAGLContext : MVGLView didMoveToSuperview @ %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
        [self willAppear];
    }
    else
    {
        NSLog(@"EAGLContext : MVGLView didMoveToSuperview (nil) @ %lx, glRenderLoop = %lx",  self.hash, self.glRenderLoop.hash);
    }
}
