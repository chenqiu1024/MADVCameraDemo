...
@interface MediaPlayerViewController () <VideoExportViewDelegate>
...
+ (instancetype) showEncoderControllerFrom:(UIViewController*)fromViewController media:(MVMedia*)media qualityLevel:(QualityLevel)qualityLevel progressBlock:(void(^)(int))progressBlock doneBlock:(void(^)(NSError*))doneBlock {
    ...
    ret.encodingDoneBlock = ^(NSString* outputFilePath, NSError* error) {
        NSLog(@"#Bug2880# MediaPlayerViewController : encodingDoneBlock #0");
        [wRet pause];
        NSLog(@"#Bug2880# MediaPlayerViewController : encodingDoneBlock #1");
...
//                {
//                    filename = [[KxMovieViewController encodedFileBaseName:wRet.media.localPath qualityLevel:wRet.encoderQualityLevel forExport:YES] lastPathComponent];
//                }
//                else
                {
...
{
    //NSLog(@"VideoEncoding: MediaPlayerViewController $ playingDoneWhileEncoding");
    if (self.isUsedAsEncoder) {
...
- (void) didPlayProgressChanged:(int)percent {
    //NSLog(@"VideoEncoding: MediaPlayerViewController $ didPlayProgressChanged : percent = %d", percent);
